...
/* Emplacements possibles des fonctions Cloudflare de TAF, dans l'ordre de
   préférence. La balise passe en tête, puis les deux conventions historiques. */
const API_BASES = [...new Set([API, "/api", "/taf/api"])];
const API_MEMO = "pic-api-base";
try {
  const memo = localStorage.getItem(API_MEMO);
  if (memo && API_BASES.includes(memo)) API = memo;
} catch (e) { /* stockage indisponible */ }

/* Journal de la dernière tentative, affiché dans les Réglages en cas d'échec.
   Sans lui, une base introuvable ne produit qu'un « échec » sans cause. */
let apiDiag = "";

async function apiFetch(path, options) {
  const ordre = [API, ...API_BASES.filter(b => b !== API)];
  const essais = [];
  let derniere = null;

  for (const base of ordre) {
    let res;
    try {
      res = await fetch(base + path, options);
    } catch (e) {
      essais.push(base + path + " → réseau injoignable");
      derniere = e;
      continue;
    }
    /* 404 et 405 signalent une fonction absente à cet emplacement, pas une
       erreur applicative : on tente le candidat suivant. Tout autre code est
       une vraie réponse du backend et doit remonter telle quelle. */
    if (res.status === 404 || res.status === 405) {
      essais.push(base + path + " → HTTP " + res.status);
      derniere = new Error("HTTP " + res.status);
      continue;
    }
    if (base !== API) {
      API = base;
      try { localStorage.setItem(API_MEMO, base); } catch (e) {}
    }
    apiDiag = "";
    return res;
  }
...
function setSyncState(state, detail) {
  ...
  if (line) {
    line.textContent = (labels[state] || state) + (detail ? " — " + detail : "") + when +
                       (state === "ok" ? " · via " + API : "");
  }
  /* Le diagnostic de résolution s'affiche sous les boutons : c'est la seule
     information qui permette de corriger un déploiement incomplet. */
  const diag = $("#sync-diag");
  if (diag) {
    diag.textContent = apiDiag || "";
    diag.hidden = !apiDiag;
  }
}
...
async function syncNow(silent) {
  ...
  try {
    ...
    /* Une panne de backend doit se voir même quand la synchro tourne en tâche
       de fond : sinon l'écran affiche « en attente » indéfiniment. */
    if (!silent || e.diag) toast("Synchronisation impossible : " + (e.message || e), "warn");
  } finally {
    syncing = false;
  }
}
...
async function enablePush() {
  ...
  try {
    ...
    const { publicKey } = await (await apiFetch("/push/vapid")).json();
