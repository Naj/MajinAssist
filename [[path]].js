...
//   GET  /api/sync/status              → état du dépôt + révision courante
//   GET  /api/sync/pull?ns=…&since=…   → lignes modifiées depuis `since`
//   POST /api/sync/push                → applique un lot de modifications
//   POST /api/sync/wipe                → vide un espace de noms (protégé)
//
// Binding attendu : DB (base D1)
...
export async function onRequest(context) {
  ...
  if (!env.DB) {
    return json({ error: 'Binding D1 « DB » absent du projet Cloudflare Pages.' }, 500);
  }
  ...
  const action = url.pathname.replace(/^\/api\/sync\/?/, '').replace(/\/$/, '');

  try {
    if (request.method === 'GET' && (action === 'status' || action === '')) {
      return await handleStatus(env.DB, url);
    }
    if (request.method === 'GET' && action === 'pull') {
      return await handlePull(env.DB, url);
    }
    if (request.method === 'POST' && action === 'push') {
      return await handlePush(env.DB, request);
    }
    if (request.method === 'POST' && action === 'wipe') {
      return await handleWipe(env.DB, request);
    }
