...
// ── Routeur ──────────────────────────────────────────────────────────────────
// Création du schéma à la première requête.
//
// Une base D1 fraîchement créée est vide : oublier de charger schema.sql donne
// un « no such table: kv » que rien dans l'interface ne laisse deviner. Les
// CREATE sont idempotents (IF NOT EXISTS) et la promesse est mémorisée, donc
// le coût se limite à trois instructions au démarrage à froid de l'isolat.
let schemaPret = null;
...
function ensureSchema(db) {
  ...
  schemaPret = db.batch([
    db.prepare(`CREATE TABLE IF NOT EXISTS kv (
      ...
      rev INTEGER NOT NULL, updated_at INTEGER NOT NULL,
      device TEXT, deleted INTEGER NOT NULL DEFAULT 0,
      PRIMARY KEY (ns, k))`),
    db.prepare('CREATE INDEX IF NOT EXISTS idx_kv_ns_rev ON kv (ns, rev)'),
    db.prepare(`CREATE TABLE IF NOT EXISTS devices (
      id TEXT PRIMARY KEY, label TEXT,
      last_seen INTEGER NOT NULL, user_agent TEXT)`),
  ]).catch((err) => {
    // Un échec ne doit pas rester mémorisé : la requête suivante doit pouvoir
    // retenter, sinon une panne passagère condamnerait l'isolat entier.
    schemaPret = null;
    throw err;
  });
  return schemaPret;
...
export async function onRequest(context) {
  ...
  try {
    ...
    return json({
      error: 'Création du schéma impossible : ' + (err?.message || String(err)) +
             ". Vérifiez que le binding DB_MAJIN pointe bien sur une base D1 accessible en écriture.",
    }, 500);
  }
